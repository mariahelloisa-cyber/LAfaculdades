...
import { EditIconLink, EmptyState, ListCard, PageHeader, PrimaryButton } from "../../AdminUI";
...
export default async function AdminNiveisPage({
  ...
  const [{ data: niveis }, { data: cursos }] = await Promise.all([
    supabase
      .from("course_niveis")
      .select("id, slug, nome, titulo, imagem_url, ordem")
            ...
            return (
              <div key={nivel.id} className="flex items-center gap-4 py-3.5">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[15px] font-bold text-navy-950">{nivel.nome}</p>
