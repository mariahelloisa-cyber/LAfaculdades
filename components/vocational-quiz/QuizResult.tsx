...
/** Símbolo oficial (logo-mark.png) como marca d'água. A caixa recorta a faixa
 *  do texto "LA FACULDADES" do arquivo, deixando só o símbolo. */
function Watermark({ className = "" }: { className?: string }) {
  return (
    <span
      aria-hidden="true"
      className={`pointer-events-none absolute aspect-[1688/2296] select-none overflow-hidden ${className}`}
    >
      <Image
...
export default function QuizResult({
  ...
  const consultorHref = `${SITE.whatsapp}?text=${encodeURIComponent(
    principal
      ? `Olá! Fiz o teste vocacional no site da LA e meu resultado apontou para ${principal.course.nome}. Quero saber mais.`
      : "Olá! Fiz o teste vocacional no site da LA e quero ajuda para escolher meu curso."
  )}`;
    ...
    >
      ...
      <div className="container-x pb-14 pt-6 lg:pb-20 lg:pt-8">
        ...
        <section
          ...
          className="mt-5 grid gap-6 rounded-[28px] bg-white/85 p-5 shadow-[0_20px_60px_rgba(15,58,92,0.08)] ring-1 ring-navy-800/5 sm:p-8 lg:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)] lg:gap-8 lg:p-10"
        >
            ...
            <ul className="mt-6 grid gap-5 md:grid-cols-2">
              {outros.map((r, i) => (
                <li
                  key={r.course.slug}
                  data-stagger
                  className="relative isolate flex flex-col overflow-hidden rounded-[22px] bg-white p-6 shadow-[0_14px_40px_rgba(15,58,92,0.06)] ring-1 ring-navy-800/5 sm:p-8"
                >
            ...
            <div className="flex items-center gap-5 lg:flex-1">
              ...
              <div>
                ...
                <p className="mt-1 text-[15px] text-sky-200">
                  Dê o próximo passo e construa o seu futuro com a LA Faculdades.
                </p>
              </div>
            </div>
